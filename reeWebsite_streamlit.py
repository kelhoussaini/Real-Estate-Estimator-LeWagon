...
print("data_to_predict  :", data_to_predict.shape)
...
X_pred = pd.DataFrame(param_dict) ##Build a dataframe for the prediction
print("X_pred  :", X_pred.shape)

model_pred = joblib.load("RandomForest.joblib")
reestimodel = model_pred.predict(X_pred)

res = dict(reestimodel=reestimodel[0])
            
     
if st.button('Prix estimé'):
    response = res #requests.get(url, params=param_dict).json()
    print("response  :", response)
    prediction = response#.json()
    col1, col2 = st.columns(2)
    col2.metric("", f"{round(response['reestimodel'], 2)}€") #col2.metric("", prediction)#f"{prediction['Prediction']}")
